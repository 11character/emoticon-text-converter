...
2. **구현 및 검증**: 
    - 코드 수정 후 `npm test -- --run` (또는 `npm run test:run`)을 실행하여 기존 기능의 회귀 여부를 확인한다. 
    - **주의**: 단순 `npm test`는 watch 모드로 실행되어 수동 종료(q 입력)가 필요하므로, 자동화된 워크플로우에서는 반드시 `--run` 플래그를 사용한다.
3. **데모 확인**: 
    - 시각적 확인이 필요한 경우 `index.html`과 `npm run dev`를 활용하여 실제 동작을 검증한다.
